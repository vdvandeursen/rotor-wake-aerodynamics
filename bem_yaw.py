...
class BladeElementModel:
    def __init__(self,
        ...
        self.blade_azimuthal_positions = np.linspace(0, 2*np.pi, blade_number+1)[:-1]  # gets blade azimuthal position
    ...
    def run_performance_calculations(self,
        ...
        self.rotor_yaw = rotor_yaw / 180 * np.pi
    ...
    def _solve_stream_tube_for_blade_element(self, section_start, twist, chord):
        ...
        while iteration_error > error_threshold:
            # Wake skew angle:
            wake_skew = (0.6 * axial_induction_factor + 1) * self.rotor_yaw

            for blade_position in self.blade_azimuthal_positions:
                # Calculate velocity and loads at blade element
                axial_velocity_rotor = self.free_stream_velocity * axial_induction_factor * (1 + 2 * np.tan(wake_skew / 2) * np.sin(blade_position))
    ...
    def _get_blade_element_loading(self, axial_velocity, tangential_velocity, chord, twist) -> tuple:
        ...
        :param axial_velocity: velocity in normal direction [m/s]
        :param tangential_velocity: velocity in tangential direction [m/s]
        :param chord: chord length of blade section [m]
        :param twist: twist angle of blade section [deg]

        :return axial_force, tangential_force, gamma. Forces are in [N/m]

        """
        velocity_squared = axial_velocity ** 2 + tangential_velocity ** 2
        inflow_angle = np.arctan2(axial_velocity, tangential_velocity)  # rad
    ...
    @staticmethod
    def _calculate_induction_factor(ct, yaw_angle=0, wake_skew=0):
        """
        This function calculates the induction factor 'a' as a function of thrust coefficient CT
        including Glauert's correction

        Follows from ct = 4a(cos(yaw_angle) + sin(yaw_angle)tan(wake_skew/2) - a*sec^2(wake_skew/2)
        """
        ...
        a_coeff = -4*(1/np.cos(wake_skew/2))**2
        b_coeff = 4*(np.cos(yaw_angle) + np.sin(yaw_angle)*np.tan(wake_skew/2))
        c_coeff = -ct[ct < ct2]

        a[ct < ct2] = (-b_coeff - np.sqrt(b_coeff**2 - 4 * a_coeff * c_coeff)) / (2 * a_coeff)
